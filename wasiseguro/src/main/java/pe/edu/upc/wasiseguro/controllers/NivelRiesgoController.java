...
public class NivelRiesgoController {
    ...
    @PostMapping("/crear")
    public ResponseEntity<NivelRiesgoListDTO> registrar(@RequestBody NivelRiesgoListDTO dto) {
        ModelMapper m = new ModelMapper();

        NivelRiesgo nivelRiesgo = m.map(dto, NivelRiesgo.class);
        NivelRiesgo nivelGuardado = nS.insert(nivelRiesgo);
        NivelRiesgoListDTO responseDTO = m.map(nivelGuardado, NivelRiesgoListDTO.class);
    ...
    public ResponseEntity<?> listarId(@PathVariable int id) {
        ...
        if (nivelRiesgo.isEmpty()) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND)
                    .body("No existe el nivel de riesgo con id: " + id);
        }

        ModelMapper m = new ModelMapper();
        NivelRiesgoListDTO dto = m.map(nivelRiesgo.get(), NivelRiesgoListDTO.class);
